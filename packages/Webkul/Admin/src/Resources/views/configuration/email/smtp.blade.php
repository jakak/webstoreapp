    ...
    <form action="{{ route('admin.configuration.email') }}" enctype="multipart/form-data" method="POST">
        ...
        <div class="page-content">
            <div class="control-group">
                ...
                <image-wrapper :button-label="'{{ __('admin::app.catalog.products.add-image-btn-title') }}'" input-name="logo" :multiple="false" :images='"{{ asset($emailConfig->logo ?? '') }}"' url-transform="no-transform"></image-wrapper>
            </div>
            ...
            <div class="control-group" :class="[errors.has('username') ? 'has-error' : '']">
                ...
                <input v-validate="'required'" class="control" id="username" name="username" 
                    ...
                    value="{{ old('username') ?: $emailConfig->username ?? '' }}" required />
